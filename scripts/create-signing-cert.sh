#!/usr/bin/env bash
# Creates a self-signed code-signing certificate so package-app.sh can sign the
# app with a STABLE identity. With a stable signature the macOS Keychain
# "Always Allow" grant (for reading Claude Code's token) persists across
# rebuilds — ad-hoc signing changes identity every build and re-prompts.
#
# Run once:
#   ./scripts/create-signing-cert.sh
#
# It does NOT need admin/sudo. codesign may show a one-time Keychain prompt the
# first time it uses the key — choose "Always Allow".
set -euo pipefail

CERT_NAME="ClaudeUsageBar Self-Signed"
...
echo "Approve the Keychain dialog once on first launch; future rebuilds won't re-ask."
